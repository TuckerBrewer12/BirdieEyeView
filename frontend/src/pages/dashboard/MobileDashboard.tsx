...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const formatHI = formatHandicapIndex;
...
export function MobileDashboard({ vm }: { vm: DashboardPageViewModel }) {
  ...
  const coloredMix = colorizeMix(l20ScoreMix, palette.scoreColors);
  const legend = mixLegend(l20ScoreMix, palette.scoreColors);
  ...
  const lastRound = recentSummaries[0]
    ? toRoundRow(recentSummaries[0], roundsById.get(recentSummaries[0].id), palette.scoreColors)
    : null;
  const lastRoundHoles = lastRound?.holes ?? [];
  const chips = lastRoundChips(lastRoundHoles, palette.scoreColors);
  const recentRoundRows = recentSummaries.map((summary) =>
    toRoundRow(summary, roundsById.get(summary.id), palette.scoreColors),
  );
