...
import { SCORE_KEYS } from "@/lib/colors";
...
const KIND_TO_HOLE: Record<NonNullable<ReturnType<typeof scoreKind>>, HoleColorKey> = {
  eagle: "eagle",
  birdie: "birdie",
  par: "par",
  bogey: "bogey",
...
/** Analytics keys, not domain ScoreKind — the API still says double_bogey. */
export function holeColorKey(
  strokes: number | null | undefined,
  par: number | null | undefined,
): HoleColorKey {
  const kind = scoreKind(strokes, par);
  return kind ? KIND_TO_HOLE[kind] : "par";
}
